...
- **Lua gauge element** - `btcui_gauge(frac, {color, prefix})` renders a progress bar cell (with trailing percentage) usable in any table or summary; backed by a reusable `gauge_element` component shared with the built-in Mempool tab's memory meter
...
- **Dashboard tab (Lua)** - `lua/tabs/dashboard.lua` provides the Dashboard as a Lua tab, demonstrating the 2+1 summary layout and gauges
...
- **Lua dialog overlay** - `btcui_dialog(opts)` opens a modal dialog built from composable parts (text/label:value/separator rows, selectable item rows, a left right arrows choice, a text input, a button row and a hint line); events are delivered to an `on_event` callback; `btcui_dialog_close()` dismisses it; dialogs are mouse-aware: click a button to activate it, click an item row to select it (second click activates, mirroring table rows), scroll the item list with the mouse wheel, and click any `[key] label` segment of the hint line to trigger that key (e.g. `[a] add node`, `[Esc] close`, `[⏎] submit`); buttons, item rows and hint segments highlight on hover
- **btcui_now()** - current unix time for age / remaining-time math (the sandbox has no `os` library)
- **Lua tab tests** - `tests/lua/test_peers_tab.lua` drives the Peers tab against a stubbed `btcui_*` API; run by CTest via a `lua-interp` binary built from the bundled Lua sources

### Changed
- The Peers tab is now a Lua tab (`lua/tabs/peers.lua`), enabled/disabled from the Settings tab like every other bundled tab (a `tab =` entry in `config.toml`); all modals (peer detail, disconnect/ban, Added Nodes, Ban List, Add Node) are built on the new `btcui_dialog` overlay, and the previously unreachable Ban/Unban address dialog is now accessible from the Ban List (`[b]`); `src/tabs/peers.cpp` and `src/tabs/peers.hpp` are no longer compiled and the background poller no longer calls `getpeerinfo`. Note: like the other bundled tabs it is off until enabled from Settings
- The Tools tab is no longer auto-injected: like Dashboard/Network/Peers it is enabled from the Settings tab (a `tab =` entry in `config.toml`); the `toolstab` config key and `--toolstab` CLI option were removed
- The mutating-RPC grants of the bundled Tools (`sendrawtransaction`/`stop`/`getprivatebroadcastinfo`) and Peers (`addnode`/`disconnectnode`/`setban`) tabs are now attached automatically by script identity whenever the bundled script is loaded — via `config.toml`, `--tab`, or the Settings tab — while staying off-limits to every other Lua script; `--settingstab` remains as the escape hatch for re-enabling a hidden Settings tab
...
- FTXUI updated from v5.0.0 to v7.0.3; the v7.0.2 fix for unbounded event-buffer growth matters here, because a receiver left over from setup pinned every terminal input event, mouse motion included, for the lifetime of the process. v7.0.2 also makes `App::PostEvent` thread safe by routing it through the task queue, which is the path this code already takes via `screen.Post()`
