...
check_source_date_epoch() {
    if [ -n "$SOURCE_DATE_EPOCH" ] && [ -z "$FORCE_SOURCE_DATE_EPOCH" ]; then
        local git_epoch
        git_epoch="$(git -c log.showSignature=false log --format=%at -1)"
        if [ "$SOURCE_DATE_EPOCH" = "$git_epoch" ]; then
            # Already matches git HEAD timestamp — no reproducibility risk.
            return
        fi
        cat << EOF
...
DISTNAME="${DISTNAME:-bitcoin-tui-${VERSION}}"
