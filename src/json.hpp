...
private:
    ...
    struct Parser {
        ...
        [[nodiscard]] char peek() { skip_ws(); return pos < src.size() ? src[pos] : '\0'; }

        char consume() {
            skip_ws();
            if (pos >= src.size()) throw exception("Unexpected end of input");
            return src[pos++];
        }
        ...
        std::string parse_string_val() {
            ...
            while (pos < src.size()) {
                ...
                switch (e) {
                    ...
                    case 't':  out += '\t'; break;
                    case 'u': {
                        if (pos + 4 > src.size()) throw exception("Truncated \\u escape");
                        unsigned cp = 0;
                        for (int i = 0; i < 4; ++i) {
                            cp <<= 4;
                            char h = src[pos++];
                            ...
                            else throw exception("Bad hex in \\u escape");
                        }
        ...
        [[nodiscard]] json parse_value() {
            ...
            if (c == 't') {
                if (src.substr(pos, 4) != "true") throw exception("Expected 'true'");
                pos += 4;
                json j; j.kind_ = Kind::Bool; j.bval_ = true; return j;
            }
            if (c == 'f') {
                if (src.substr(pos, 5) != "false") throw exception("Expected 'false'");
                pos += 5;
                json j; j.kind_ = Kind::Bool; j.bval_ = false; return j;
            }
            if (c == 'n') {
                if (src.substr(pos, 4) != "null") throw exception("Expected 'null'");
                pos += 4;
                return json{};
            }
    ...
    [[nodiscard]] std::string dump_impl(int indent, int depth) const {
        ...
        switch (kind_) {
            ...
            case Kind::Bool:   return bval_ ? "true" : "false";
            case Kind::Int:    return std::to_string(ival_);
            case Kind::Float: {
                if (!std::isfinite(fval_)) return "null";
                std::ostringstream ss;
                ss << std::setprecision(17) << fval_;
                return ss.str();
            }
            case Kind::String: return quote(sval_);
            case Kind::Array: {
                if (aval_.empty()) return "[]";
                std::string out = "[";
...
public:
    ...
    [[nodiscard]] bool is_number()         const noexcept { return kind_ == Kind::Int || kind_ == Kind::Float; }
    ...
    [[nodiscard]] T get() const {
        if constexpr (std::same_as<T, bool>) {
            if (kind_ != Kind::Bool) throw exception("get<bool> on non-bool");
            return bval_;
        } else if constexpr (std::integral<T>) {
            if (kind_ == Kind::Int)   return static_cast<T>(ival_);
            if (kind_ == Kind::Float) return static_cast<T>(fval_);
            ...
            if (kind_ != Kind::String) throw exception("get<string> on non-string");
            return sval_;
        }
    ...
    json& operator[](const std::string& key) {
        ...
        if (kind_ != Kind::Object) throw exception("operator[string] on non-object");
        return oval_[key];
    }

    [[nodiscard]] const json& operator[](const std::string& key) const {
        static const json null_val{};
        if (kind_ != Kind::Object) return null_val;
        auto it = oval_.find(key);
        return it != oval_.end() ? it->second : null_val;
    }

    json& operator[](size_t i) {
        if (kind_ != Kind::Array) throw exception("operator[size_t] on non-array");
        return aval_[i];
    }
    ...
    template<typename T>
    [[nodiscard]] T value(const std::string& key, const T& def) const {
        if (kind_ != Kind::Object) return def;
        auto it = oval_.find(key);
        if (it == oval_.end() || it->second.is_null()) return def;
    ...
    [[nodiscard]] std::string value(const std::string& key, const char* def) const {
        ...
        auto it = oval_.find(key);
        if (it == oval_.end() || it->second.is_null()) return fallback;
        if (it->second.kind_ != Kind::String) return fallback;
        return it->second.sval_;
    }

    [[nodiscard]] size_t size() const noexcept {
        if (kind_ == Kind::Array)  return aval_.size();
        if (kind_ == Kind::Object) return oval_.size();
        return 0;
    }
    ...
    [[nodiscard]] static json array() {
        json j; j.kind_ = Kind::Array; return j;
    }
    [[nodiscard]] static json object() {
        json j; j.kind_ = Kind::Object; return j;
    }
