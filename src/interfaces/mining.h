...
//
// Note: waitTipChanged is declared to return BlockRef (not std::optional),
// matching v31.0's capnp wire format which does not carry a `hasResult`
// companion field for that method. Callers should treat a returned BlockRef
// with a null hash as "no new tip / interrupted".
class Mining
{
public:
    ...
    // ordinal 2 — has companion `hasResult` field on the wire.
    virtual std::optional<BlockRef> getTip() = 0;
    // ordinal 3 — blocks until tip != current_tip or timeout/interrupt fires.
    virtual BlockRef waitTipChanged(
        uint256 current_tip,
